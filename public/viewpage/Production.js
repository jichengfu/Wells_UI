...
export function addEventListeners() {
  Element.menuProduction.addEventListener("click", async () => {
    history.pushState(null, null, Routes.routePath.PRODUCTION);
    production_page();
  });
}

export async function production_page() {
  if (!Auth.currentUser) {
...
export function buildProductionScreen() {
  let html = "";
  ...
  Element.menuProduction.style.background = "#f1741a";
}
